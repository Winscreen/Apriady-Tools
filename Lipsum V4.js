...
(function($) {
	...
	if (typeof (ls) !== 'undefined') {
		...
		$resetBtn.button({
			...
			$confirmBox.html('Semua Data Yang Disimpan Akan Dihapus').dialog({
				title: 'Dihapus !',
				modal: true,
				buttons: {
					"Undo": function() {
	...
	$addVideo.on("click", function() {
		$area.prepend('<div class="sortable hidden"><div class="separator align-center"><iframe width="560" height="315"src="' + $sc1.val() + '"frameborder="0" allowfullscreen></iframe></div><input type="checkbox" class="remove" /></div>');
		fallDown();
		removePart();
		toggleClass();
	});
	...
	$removeAll.button({
		...
		if ($('.sortable').find(':checked').length) {
			...
			$confirmBox.html('Silakan Pilih Object!').dialog({
				title: "Peringatan!!!",
				modal: true,
				buttons: {
					"OK": function() {
	...
	$area.on("click", ".dels", function() {
		$confirmBox.html('<span class="ui-icon ui-icon-alert" style="float:left;margin:0px 10px 0px 0px;"></span>Hey, this will not be returned. Are you sure?!').dialog({
			...
			buttons: {
				...
				"Really?": function() {
					$confirmBox.html('<img src="http://4.bp.blogspot.com/-tMCwBZ51OAE/VlcifsSSB9I/AAAAAAAADDM/Duof1B7-H-w/s1600/Apriady%2BLogo.ICO" style="float:left;margin:0px 10px 0px 0px;" /> Soo Amazing Hiihii ... Have fun .. <a class="jokes" href="#">add a new paragraph</a>').dialog({
						buttons: {
	...
	$extrasBtn.button({
		...
		$area.slideUp(400, function() {
			$('<div id="loading-helper"><strong class="ui-state-error">Loading...</strong></div>').appendTo($preview).load("http://apriady-tools.blogspot.co.id/p/extras.html #extras-container");
		});
		$(this).button({
			label: "Back",
			icons: {
